...
import 'package:aptech_clifton/pages/faculty/side_drawer.dart';
import 'faculty_services/FacultyService.dart';

class FacultyDashboard extends StatelessWidget {
  final String facultyId = 'TWOBY0Wt5QYzBL5JDGv7bDHlM1g1';

  @override
  Widget build(BuildContext context) {
    final facultyService = Provider.of<FacultyService>(context);
    ...
    return Scaffold(
      ...
      drawer: FutureBuilder<Map<String, dynamic>?>(
        future: facultyService.getFacultyData(facultyId),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Drawer(
              child: Center(child: Text('Faculty not found')),
            );
          } else {
            final facultyData = snapshot.data!;
            return SideDrawer(
              facultyName: facultyData['name'] ?? 'No Name',
              facultyImageUrl: facultyData['imageUrl'] ?? 'assets/images/default_image.png',
            );
          }
        },
      ),
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _buildHeader(facultyId, facultyService),
  ...
  Widget _buildHeader(String facultyId, FacultyService facultyService) {
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Center(child: Text('Faculty not found'));
        } else {
          final facultyData = snapshot.data!;
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
