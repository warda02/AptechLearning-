...
import 'faculty_services/FacultyService.dart'; // Ensure this service is created
...
class _ManageSessionsPageState extends State<ManageSessionsPage> {
  ...
  Future<void> _fetchSessionsData() async {
    try {
      List<Session> data = await _facultyService.fetchSessionsData();
      setState(() {
        sessionsData = data;
        isLoading = false;
      });
          ...
          actions: [
            ...
            TextButton(
              ...
              style: TextButton.styleFrom(
                foregroundColor: Color(0xFFFEBE10), // Changed from 'foregroundColor'
              ),
            ),
          ],
        );
  ...
  Future<void> _addSession({
    ...
    try {
      await _facultyService.addSession(
        title: title,
        date: date,
        time: time,
        faculty: faculty,
  ...
  Future<void> _updateSession({
    ...
    try {
      await _facultyService.updateSession(
        id: id,
        title: title,
        date: date,
        time: time,
  ...
  Future<void> _deleteSession(String id) async {
    try {
      await _facultyService.deleteSession(id);
      _fetchSessionsData();
    } catch (e) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                trailing: Row(
                  ...
                  children: [
                    IconButton(
                      icon: Icon(Icons.edit, color: Color(0xFFFEBE10)), // Fixed color
